...
const writeFs = function (title, content) {
    // 添加数据
    fs.createWriteStream(path.join(__dirname, `/book/${title}.txt`))
    fs.appendFile(path.join(__dirname, `/book/${title}.txt`), title, function (err) {
        if (err) throw err;
    });
    fs.appendFile(path.join(__dirname, `/book/${title}.txt`), content, function (err) {
        if (err) {
            console.log(err)
        } else {
